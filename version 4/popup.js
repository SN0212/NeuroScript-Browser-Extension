...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- Puter.js Summarization logic ---
    summarizeBtn.addEventListener("click", async () => {
        const textToSummarize = inputText.value.trim();
        if (!textToSummarize) {
            summaryOutput.textContent = "Please paste some text to summarize.";
            return;
        }
        ...
        try {
            // Call Puter.js chat with a prompt to summarize the text
            ...
            if (response?.choices?.[0]?.message?.content) {
                summaryOutput.textContent = response.choices[0].message.content.trim();
            } else {
                summaryOutput.textContent = "Sorry, no summary was generated.";
            }
        } catch (error) {
            console.error("Puter.js summarization error:", error);
            summaryOutput.textContent = "Error occurred during summarization. Please try again.";
        }
    });
});
