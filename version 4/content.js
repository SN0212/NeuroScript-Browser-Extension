...
document.addEventListener("DOMContentLoaded", () => {
    ...
    inputArea.placeholder = "Paste text here to summarize...";
    ...
    // Create output area for summary
    const outputArea = document.createElement("div");
    outputArea.style.marginTop = "12px";
    outputArea.style.padding = "10px";
    outputArea.style.backgroundColor = "#e0e0e0";
    ...
    // Append container at the top of the body or wherever you want
    document.body.insertBefore(container, document.body.firstChild);

    // Puter.js client setup - replace YOUR_API_KEY with your real API key securely
    const puter = new Puter({ apiKey: "YOUR_API_KEY" });

    summarizeBtn.addEventListener("click", async () => {
        const textToSummarize = inputArea.value.trim();
        if (!textToSummarize) {
            outputArea.textContent = "Please paste some text to summarize.";
            return;
        }
        outputArea.textContent = "Summarizing...";
        try {
            // Call Puter.js summarize AI
            ...
            outputArea.textContent = response.choices[0].message.content.trim();
        } catch (error) {
            outputArea.textContent = "Error during summarization: " + error.message;
        }
    });
});

// Add dropdown for ambient music
document.addEventListener("DOMContentLoaded", () => {
    let musicDropdown = document.createElement("select");
    musicDropdown.innerHTML = `
        <option value="">Select Ambient Music</option>
        <option value="white_noise">White Noise</option>
        <option value="brown_noise">Brown Noise</option>
        <option value="lofi">LoFi</option>
    `;
    musicDropdown.addEventListener("change", (event) => playMusic(event.target.value));

    let musicControls = document.createElement("button");
    musicControls.textContent = "Pause Music";
...
function playMusic(track) {
    ...
    audio.src = chrome.runtime.getURL(`music/${track}.mp3`);
    audio.loop = true;
    audio.play();
}
